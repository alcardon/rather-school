      ...
      try {
        selectedCourses.map(async (course_id: String) => {
          let { error } = await supabase
            .from("student_enrollment_grade")
            .insert({ student_id: newStudentId, course_id });
        });
      ...
      try {
        ...
        siblingsIds.map(async (sibling_id) => {
          ...
          await supabase
            ...
            .insert({ student_id: newStudentId, sibling_id });
          await supabase
            .from("sibling_relationships")
            .insert({ student_id: sibling_id, sibling_id: newStudentId });
        });
        ...
        <div className="flex-auto px-4 py-10 pt-0 lg:px-10">
          <form onSubmit={handleSubmit} method="POST">
            ...
            <div className="flex flex-wrap">
              <div className="w-full px-4 lg:w-6/12">
                <div className="relative w-full mb-3">
                  ...
                  <RelativesSelect
                    studentRelatives={studentRelatives}
                    setStudentRelatives={setstudentRelatives}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end w-full pr-3">
              <button
                type="submit"
                className="px-6 py-3 mb-1 mr-1 text-sm font-bold text-white uppercase transition-all duration-150 ease-linear rounded shadow outline-none bg-emerald-500 hover:shadow-lg focus:outline-none active:bg-emerald-600"
              >
                Save
              </button>
            </div>
